...
class RunAnalysis:
    ...
    def loadSystematicsReweightsEnvelopeRMS(self):
        ...
        for sampleName in self.dfs.keys():
            for index in self.dfs[sampleName].keys():
                df = self.dfs[sampleName][index]["df"]
                parent = self.dfs[sampleName][index]['parent'] if 'parent' in self.dfs[sampleName][index].keys() else None
                columnNames = self.dfs[sampleName][index]["columnNames"]
                nuisances = self.nuisances
                # nuisance key is not used
                for _, nuisance in list(nuisances.items()):
                    if (sampleName not in nuisance.get("samples", {sampleName: []})) and (parent not in nuisance.get("samples", {sampleName: []})):
                        continue
                    if nuisance.get("type", "") == "shape":
                        if (
                            ...
                            weights = nuisance["samples"].get(sampleName if parent is None else parent, None)
                            if weights is not None:
                                variedNames = []
                                variedTags = []
        ...
        for sampleName in sampleNames:
            ...
            for subsample in list(_sample[5].keys()):
                ...
                for index in self.dfs[sampleName].keys():
                    ...
                    self.dfs[sampleName + "_" + subsample][index]["usedVariables"] = self.dfs[
                        sampleName
                    ][index]["usedVariables"]
    ...
    def run(self):
        ...
        #print(self.cuts)
        #print(self.results)
        #print('--> Printing split subsamples')
        #self.splitSubsamples()
